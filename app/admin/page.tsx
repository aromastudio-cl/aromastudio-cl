...
type SiteSettings={phone:string;whatsapp_number:string;whatsapp_enabled:boolean;instagram_url:string;facebook_url:string;tiktok_url:string;youtube_url:string;hero_desktop_url:string;hero_desktop_path:string;hero_mobile_url:string;hero_mobile_path:string;hero_title_accent:string;hero_title:string;hero_description:string;announcement_text:string;announcement_enabled:boolean;stores_eyebrow:string;stores_title:string;stores_content_eyebrow:string;stores_content_title:string;stores_content_text:string;stores_image_url:string;stores_image_path:string;privacy_title:string;privacy_content:string;terms_title:string;terms_content:string;shipping_title:string;shipping_content:string};
...
export default function Admin() {
  ...
  const [configSection, setConfigSection] = useState<"encabezado" | "portada" | "contacto" | "redes" | "preguntas" | "sucursales" | "politicas">("encabezado");
  ...
  const [siteSettings, setSiteSettings] = useState<SiteSettings>({ phone: "", whatsapp_number: "", whatsapp_enabled: true, instagram_url: "", facebook_url: "", tiktok_url: "", youtube_url: "", hero_desktop_url:"", hero_desktop_path:"", hero_mobile_url:"", hero_mobile_path:"", hero_title_accent:"Descubre el aroma perfecto", hero_title:"para cada espacio", hero_description:"Descubre fragancias que transforman tu hogar y tu día a día.", announcement_text:"ENVÍOS A TODO CHILE", announcement_enabled:true, stores_eyebrow:"TIENDAS OFICIALES", stores_title:"Visítanos en nuestras tiendas", stores_content_eyebrow:"AROMA STUDIO", stores_content_title:"Encuentra tu aroma favorito", stores_content_text:"Visita nuestros puntos de venta y descubre una selección de productos y aromas pensados para transformar tus espacios. Nuestro equipo estará disponible para orientarte.", stores_image_url:"", stores_image_path:"", privacy_title:"Política de privacidad", privacy_content:"", terms_title:"Términos y condiciones", terms_content:"", shipping_title:"Envíos y devoluciones", shipping_content:"" });
  ...
  useEffect(() => {
    ...
    supabase.from("site_settings").select("phone,whatsapp_number,whatsapp_enabled,instagram_url,facebook_url,tiktok_url,youtube_url,hero_desktop_url,hero_desktop_path,hero_mobile_url,hero_mobile_path,hero_title_accent,hero_title,hero_description,announcement_text,announcement_enabled,stores_eyebrow,stores_title,stores_content_eyebrow,stores_content_title,stores_content_text,stores_image_url,stores_image_path,privacy_title,privacy_content,terms_title,terms_content,shipping_title,shipping_content").eq("id",1).maybeSingle().then(({data})=>{if(data){setSiteSettings(data as SiteSettings);setHeroDesktopPreview(data.hero_desktop_url||DEFAULT_HERO_DESKTOP);setHeroMobilePreview(data.hero_mobile_url||DEFAULT_HERO_MOBILE);setLocationPreview(data.stores_image_url||DEFAULT_LOCATION_IMAGE);}});
  ...
  return (
    <main className="admin-shell">
      ...
      <section className="admin-content">
        ...
        {tab === "Configuración" && <><nav className="configuration-submenu" aria-label="Secciones de configuración">{[{id:"encabezado",label:"Encabezado"},{id:"portada",label:"Portada"},{id:"contacto",label:"Contacto"},{id:"redes",label:"Redes sociales"},{id:"preguntas",label:"Preguntas frecuentes"},{id:"sucursales",label:"Sucursales"},{id:"politicas",label:"Políticas"}].map(item=><button type="button" className={configSection===item.id?"active":""} onClick={()=>setConfigSection(item.id as typeof configSection)} key={item.id}>{item.label}</button>)}</nav>
        {(configSection === "encabezado" || configSection === "portada" || configSection === "contacto" || configSection === "redes" || configSection === "politicas") && <section className="admin-table configuration-panel"><header><div><p>CONFIGURACIÓN</p><h2>{configSection==="encabezado"?"Franja superior":configSection==="portada"?"Portada del sitio":configSection==="contacto"?"Teléfono y WhatsApp":configSection==="politicas"?"Políticas del sitio":"Redes sociales"}</h2></div></header>
          <form className="site-settings-form" onSubmit={saveSiteSettings}>
            ...
            {configSection==="portada"&&<div className="site-settings-form__hero"><div><span>PORTADA DEL SITIO</span><h3>Contenido del hero</h3><p>Edita los textos principales de la portada y administra las fotografías para escritorio y celular.</p><div className="hero-copy-fields"><label>TÍTULO DESTACADO<input maxLength={80} value={siteSettings.hero_title_accent} onChange={event=>setSiteSettings({...siteSettings,hero_title_accent:event.target.value})}/></label><label>TÍTULO PRINCIPAL<input maxLength={80} value={siteSettings.hero_title} onChange={event=>setSiteSettings({...siteSettings,hero_title:event.target.value})}/></label><label>DESCRIPCIÓN<textarea rows={4} maxLength={220} value={siteSettings.hero_description} onChange={event=>setSiteSettings({...siteSettings,hero_description:event.target.value})}/></label></div></div><div className="hero-image-fields"><label><b>ESCRITORIO</b><span>Proporción óptima: 1920 × 640 px · 3:1</span>{heroDesktopPreview&&<figure><em>IMAGEN ACTUAL</em><Image src={heroDesktopPreview} alt="Fotografía actual del hero de escritorio" fill unoptimized/></figure>}<input type="file" accept="image/jpeg,image/png,image/webp" onChange={event=>selectHeroImage(event,"desktop")}/><i>{compressing?"Procesando…":"Reemplazar fotografía"}</i>{heroDesktopPreview&&<button className="hero-download" type="button" onClick={event=>downloadImage(event,heroDesktopPreview,"hero-aroma-studio-escritorio")}><Download aria-hidden="true"/> DESCARGAR IMAGEN</button>}</label><label><b>CELULAR</b><span>Proporción óptima: 1080 × 1920 px · 9:16</span>{heroMobilePreview&&<figure className="mobile"><em>IMAGEN ACTUAL</em><Image src={heroMobilePreview} alt="Fotografía actual del hero móvil" fill unoptimized/></figure>}<input type="file" accept="image/jpeg,image/png,image/webp" onChange={event=>selectHeroImage(event,"mobile")}/><i>{compressing?"Procesando…":"Reemplazar fotografía"}</i>{heroMobilePreview&&<button className="hero-download" type="button" onClick={event=>downloadImage(event,heroMobilePreview,"hero-aroma-studio-movil")}><Download aria-hidden="true"/> DESCARGAR IMAGEN</button>}</label></div></div>}
            ...
            {configSection==="politicas"&&<div className="site-settings-form__group policies-settings"><h3>Documentos legales</h3><p>Edita los títulos y contenidos que se publican en los enlaces legales del footer.</p><label>Título de privacidad<input required value={siteSettings.privacy_title} onChange={e=>setSiteSettings({...siteSettings,privacy_title:e.target.value})}/></label><label>Política de privacidad<textarea required rows={9} value={siteSettings.privacy_content} onChange={e=>setSiteSettings({...siteSettings,privacy_content:e.target.value})} placeholder="Escribe aquí la política de privacidad…"/></label><label>Título de términos<input required value={siteSettings.terms_title} onChange={e=>setSiteSettings({...siteSettings,terms_title:e.target.value})}/></label><label>Términos y condiciones<textarea required rows={9} value={siteSettings.terms_content} onChange={e=>setSiteSettings({...siteSettings,terms_content:e.target.value})} placeholder="Escribe aquí los términos y condiciones…"/></label><label>Título de envíos y devoluciones<input required value={siteSettings.shipping_title} onChange={e=>setSiteSettings({...siteSettings,shipping_title:e.target.value})}/></label><label>Política de envíos y devoluciones<textarea required rows={9} value={siteSettings.shipping_content} onChange={e=>setSiteSettings({...siteSettings,shipping_content:e.target.value})} placeholder="Escribe aquí la política de envíos y devoluciones…"/></label></div>}
