...
type Product = { id: string; name: string; family: string; category: string; price: number; note: string; image: string; stock: number };
...
export default function Storefront() {
  ...
  useEffect(() => {
    supabase
      .from("products")
      .select("id,name,scent_notes,price_clp,stock,categories(name),product_images(image_url,is_primary,sort_order)")
      .eq("active", true)
      .eq("featured", true)
      ...
      .then(({ data, error }) => {
        ...
        setProducts((data ?? []).map((item: any) => ({
          id: item.id,
          name: item.name,
          family: item.categories?.name ?? "Aroma Studio",
          category: item.categories?.name ?? "Otros",
          price: item.price_clp,
          note: item.scent_notes ?? "",
          stock: item.stock,
          image: item.product_images?.find((image: any) => image.is_primary)?.image_url
  ...
  return <main>
    ...
    <section className="catalog" id="productos"><div className="section-title"><p>PRODUCTOS DESTACADOS</p><h2>Encuentra tu favorito</h2><i/></div><div className="filters" id="aromas">{categories.map(category => <button key={category} className={activeCategory === category ? "active" : ""} onClick={() => setActiveCategory(category)}>{category.toUpperCase()}</button>)}</div>
      <div className="product-list">{visible.map(product => <article className="product-card" key={product.id}><div className="product-photo"><Image src={product.image} alt={`Aromatizante ${product.name} de Aroma Studio`} fill sizes="(max-width: 850px) 50vw, 20vw" unoptimized={product.image.startsWith("http")}/></div><h3>{product.name}</h3><p>{product.family}</p><strong>{money(product.price)}</strong><button disabled={product.stock === 0} onClick={() => add(product.id)}>{product.stock === 0 ? "SIN STOCK" : "AGREGAR AL CARRITO"}</button></article>)}</div>
    ...
    <section className="about" id="nosotros"><article><p>SOBRE NOSOTROS</p><h2>Pasión por los aromas</h2><span>En Aroma Studio seleccionamos cuidadosamente cada esencia para ofrecer productos de alta calidad que transforman tus espacios y momentos.<br/><br/>Creemos que cada aroma cuenta una historia y despierta emociones.</span><div className="store-locations" aria-label="Nuestros locales"><strong>VISÍTANOS EN NUESTROS DOS LOCALES</strong><span><MapPin aria-hidden="true"/>Espacio Urbano Plaza Maipú</span><span><MapPin aria-hidden="true"/>Espacio Urbano Las Rejas</span></div><a className="btn btn-outline" href="mailto:hola@aromastudio.cl">CONÓCENOS</a></article><div/></section>
