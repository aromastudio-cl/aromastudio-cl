...
export async function generateMetadata(): Promise<Metadata> { const h = await headers(); const host = h.get("host") ?? "localhost:3000"; const protocol = host.includes("localhost") ? "http" : "https"; const image = `${protocol}://${host}/og.png`; return { title: "Aroma Studio | Aromatizantes ambientales para ti, tu negocio y emprendimiento", description: "Aromatizantes ambientales premium y venta mayorista en Chile.", icons: { icon: "/favicon-logo.png" }, openGraph: { title: "Aroma Studio", description: "Aromas que despiertan emociones.", images: [{ url: image, width: 1792, height: 933 }] }, twitter: { card: "summary_large_image", images: [image] } }; }
