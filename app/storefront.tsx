...
const mobileCategories = [
  { slug: "home-spray", name: "Home Spray", description: "Impacto inmediato: renueva tu espacio en un solo gesto.", image: "/products/generated/home-spray-mango.webp" },
  { slug: "mikados-varilla", name: "Difusor de Varillas", description: "Aroma constante que acompaña tus espacios cada día.", image: "/products/generated/mikados-varilla-mango.webp" },
  { slug: "humidificadores", name: "Humidificadores", description: "Una bruma delicada para crear ambientes de bienestar.", image: "/products/generated/humidificadores-mango.webp" },
  { slug: "esencias-puras", name: "Esencias Puras", description: "Fragancias concentradas para personalizar cada momento.", image: "/products/generated/esencias-puras-mango.webp" },
  { slug: "difusor-auto", name: "Difusor para Auto", description: "Tu aroma favorito también puede acompañarte en el camino.", image: "/products/generated/difusor-auto-mango.webp" },
];
  ...
  return <main className="storefront">
    <section className="mobile-public-cover" aria-label="Portada de Aroma Studio">
      ...
      <Image
        ...
        src="/mobile-home-spray-reference-match.png"
    ...
    <section className="catalog" id="productos"><div className="mobile-catalog-intro"><span>EXPLORA</span><h2>Categorías de productos</h2><p>Un mismo ritual, distintas maneras de vivirlo. Elige el formato que mejor acompaña tu espacio.</p></div><div className="mobile-category-carousel" aria-label="Categorías de productos">{mobileCategories.map(category => { const realProduct = products.find(product => product.categorySlug === category.slug); const image = realProduct?.image ?? category.image; return <article key={category.name}><Link href={realProduct?.href ?? "/tienda"}><span className="mobile-category-carousel__image"><Image src={image} alt={`${category.name} de Aroma Studio`} fill sizes="64vw" unoptimized={image.startsWith("http")}/></span><h3>{category.name}<b aria-hidden="true">→</b></h3><p>{category.description}</p></Link></article>; })}</div><div className="section-title"><p>PRODUCTOS DESTACADOS</p><h2>Encuentra tu favorito</h2><i/></div><div className="filters" id="aromas">{categories.map(category => <button key={category} className={activeCategory === category ? "active" : ""} onClick={() => setActiveCategory(category)}>{category.toUpperCase()}</button>)}</div>
