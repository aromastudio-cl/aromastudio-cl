...
export default function Admin() {
  ...
  useEffect(() => {
    ...
    supabase.from("site_settings").select("phone,whatsapp_number,whatsapp_enabled,instagram_url,facebook_url,tiktok_url,youtube_url,hero_desktop_url,hero_desktop_path,hero_mobile_url,hero_mobile_path,announcement_text,announcement_enabled,stores_eyebrow,stores_title,stores_content_eyebrow,stores_content_title,stores_content_text,stores_image_url,stores_image_path,privacy_title,privacy_content,terms_title,terms_content,shipping_title,shipping_content").eq("id",1).maybeSingle().then(({data})=>{if(data){setSiteSettings(data as SiteSettings);setHeroDesktopPreview(data.hero_desktop_url||DEFAULT_HERO_DESKTOP);setHeroMobilePreview(data.hero_mobile_url||DEFAULT_HERO_MOBILE);setLocationPreview(data.stores_image_url||DEFAULT_LOCATION_IMAGE);}});
