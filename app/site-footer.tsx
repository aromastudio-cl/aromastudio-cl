...
export default function SiteFooter() {
  ...
  useEffect(()=>{supabase.from("site_settings").select("phone,whatsapp_number,instagram_url,facebook_url,tiktok_url,youtube_url").eq("id",1).maybeSingle().then(({data})=>{if(data)setContact({phone:data.phone||"",whatsapp:data.whatsapp_number||"",instagram:data.instagram_url||"",facebook:data.facebook_url||"",tiktok:data.tiktok_url||"",youtube:data.youtube_url||""})})},[]);
  return <footer className="site-footer">
    ...
    <nav aria-label="Productos"><strong>PRODUCTOS</strong><Link href="/productos">Catálogo de productos</Link></nav>
    <nav aria-label="Información"><strong>INFORMACIÓN</strong><Link href="/nosotros">Sobre nosotros</Link></nav>
